...
public class NetworkClient {
    ...
    public static String sendAndReceive(String message) {
        // 1. Kiểm tra ống nước. Chưa cắm thì phải cắm ngay!
        if (!isConnected()) {
            System.out.println("Client: Ống nước chưa nối, đang thử nối lại với Server...");
            if (!connect("127.0.0.1", 5000)) {
                return "LOGIN_FAIL|Không thể kết nối đến Server. Vui lòng bật Server!";
            }
        }

        // 2. Chốt chặn an toàn: Lỡ connect() rồi mà 'out' vẫn null
        if (out == null) {
            return "ERROR|Lỗi cấu hình Socket nội bộ, biến out bị null";
        }

        // 3. Tiến hành gửi dữ liệu
        try {
            out.println(message);
            return in.readLine();
        } catch (IOException e) {
            return "ERROR|Đứt cáp mạng giữa chừng rồi!";
        }
    }
